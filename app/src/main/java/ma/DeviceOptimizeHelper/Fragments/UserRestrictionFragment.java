...
import ma.DeviceOptimizeHelper.Services.UserService;
...
import ma.DeviceOptimizeHelper.Utils.ResourcesUtils;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        try {
            if (sharedPreferences.getBoolean("isGrantDhizuku", false)) {
                DhizukuUserServiceArgs args = new DhizukuUserServiceArgs(new ComponentName(BaseApplication.getContext(), UserService.class));

                try {
                    boolean isOK = Dhizuku.bindUserService(args, new ServiceConnection() {
                        @Override
                    ...
                    if (!isOK) Toast.makeText(context, "绑定失败", Toast.LENGTH_SHORT).show();
