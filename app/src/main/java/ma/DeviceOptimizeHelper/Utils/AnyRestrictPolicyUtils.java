...
import static ma.DeviceOptimizeHelper.Main.getAppName;
...
import ma.DeviceOptimizeHelper.Main;
...
public class AnyRestrictPolicyUtils {
    ...
    private static final ma.DeviceOptimizeHelper.Utils.Enterprise.DeviceManager ideviceManager = ServiceManager.getEnterpriseManager().getDeviceManager();

    private static final ma.DeviceOptimizeHelper.Utils.Enterprise.ApplicationManager iapplicationManager = ServiceManager.getEnterpriseManager().getApplicationManager();
        ...
        try {
            ...
            for (String packageName : ServiceManager.getPackageManager().getAllPackages()){
                if (applicationManager.getApplicationSettings(packageName) != ApplicationManager.FLAG_DEFAULT){
                    System.out.println("已撤销 "+getAppName(packageName)+" 被授予的特权 --> "+(sAppPrivilegeArray.get(applicationManager.getApplicationSettings(packageName)) != null ? sAppPrivilegeArray.get(applicationManager.getApplicationSettings(packageName)) : applicationManager.getApplicationSettings(packageName)));
            ...
            for (AccessibilityServiceInfo serviceInfo : ServiceManager.getAccessibilityManager().getEnabledAccessibilityServiceList(AccessibilityServiceInfo.FEEDBACK_ALL_MASK, UserManager.myUserId())){
                ...
                System.out.println("已撤销授予 "+getAppName(componentName.getPackageName())+" 的无障碍权限");
            }
