...
public class ReflectUtils {
    ...
    public static Map<String, Field> cacheFields(Class<?> clazz) {
    ...
    public static Object callObjectMethod(Object obj, String methodName, Object... args) throws InvocationTargetException, IllegalAccessException {
    ...
    public static int getResIdReflect(String key) {
        ...
        try {
            return (int) getObjectField(R.string.class, key);
        } catch (Throwable e) {
            AppGlobals.getMMKV("crash_log").encode("getResIdReflect", e.getMessage());
            return -1;
        }
    }
    ...
    private static final Map<String, Class<?>> primitiveWrapperMap = new ConcurrentHashMap<String, Class<?>>(){{
        ...
        put("android.app.ContextImpl", Context.class);
    }};
    ...
    public static Object getObjectField(Class<?> cls, String fieldName) {
        try {
    ...
    public static Object getObjectField(String clsName, ClassLoader loader, String fieldName) {
        try {
            Field declaredField;
            if (loader == null) {
                declaredField = Class.forName(clsName).getDeclaredField(fieldName);
            }else {
                declaredField = loader.loadClass(clsName).getDeclaredField(clsName);
            }
            declaredField.setAccessible(true);
            return declaredField.get(null);
        } catch (Throwable e) { throw new RuntimeException(e); }
